...
def evaluate_model(model_part0, model_part1, model_part2, test_loader, device, save_dir=None):
    ...
    print(classification_report(all_labels, all_preds, 
                               target_names=[f"Class {i}" for i in range(DatasetConfig.TREC_NUM_CLASSES)], zero_division=0))
    ...
    if save_dir:
        ...
        sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", 
                    xticklabels=[f"Class {i}" for i in range(DatasetConfig.TREC_NUM_CLASSES)],
                    yticklabels=[f"Class {i}" for i in range(DatasetConfig.TREC_NUM_CLASSES)])
...
def main():
    ...
    # 模型路径 - 请根据实际情况修改
    model_part0_path = "/mnt/workspace/v6/aggregated_models/weighted_part0.pt"
    model_part1_path = "/mnt/workspace/v6/aggregated_models/weighted_part1.pt"
    model_part2_path = "/mnt/workspace/v6/aggregated_models/weighted_part2.pt"
