...
class FLServer:
    ...
    def _preload_test_data(self):
        ...
        self.test_dataloader = list(get_test_dataloaders(batch_size=TrainingConfig.TEST_BATCH_SIZE))
    ...
    def save_model(self, server_id, accuracy):
        ...
        torch.save(self.server_instances[server_id]['model'].state_dict(), model_path)
    ...
    def train_client(self, server_id):
        ...
        try:
            ...
            self.server_instances[server_id]['status'] = 'training'
            self.currently_training = True
            self.server_instances[server_id]['model'] = ModelPart1().to(self.device)
            if SystemConfig.SERVER_MODEL_PART_BACKFORWARD:
                self.server_instances[server_id]['optimizer'] = torch.optim.AdamW(self.server_instances[server_id]['model'].parameters(), lr=TrainingConfig.LEARNING_RATE)
                self.server_instances[server_id]['model'].train()
            else:
                self.server_instances[server_id]['model'].eval()
            while True:
                data = receive_full_data(conn)
                if data is None:
                    break
                if 'epoch_metrics' in data:
                    ...
                    self.server_instances[server_id]['current_epoch'] = epoch_num
                    ...
                    print(f"📊 Server {server_id} - Epoch {epoch_num}: "
                        f"Loss: {loss:.4f} | Train Acc: {accuracy:.2f}%")
                    if SystemConfig.SERVER_MODEL_PART_BACKFORWARD:
                        if accuracy >= max(self.train_acc_history[server_id]):
                            self.save_model(server_id, accuracy)
                    

                elif 'training_complete' in data:
                    print(f"🏁 Training complete for server {server_id}")
                    del self.server_instances[server_id]['model']
                    if SystemConfig.SERVER_MODEL_PART_BACKFORWARD:
                        del self.server_instances[server_id]['optimizer']
